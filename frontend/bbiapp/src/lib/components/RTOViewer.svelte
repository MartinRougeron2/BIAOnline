<script lang="ts">
  ...
  import { Input, Range } from "flowbite-svelte";
    ...
    function getLabels (impacts: ImpactActivity[], services: ServiceActivity[]) : number[] {
        let scales: number[] = [0];
    ...
    let impacts: ImpactType[] = activity.impacts.map(impact => {
    ...
    function factoryImpact(data: number[], baseScale: number[], newScale: number[]) {
    ...
    $: xAxis = xAxisImpacts.concat(xAxisServices).concat([0, activity.RTO]).sort((a, b) => a - b);
    ...
    $: options = {
        ...
        scales: {
            ...
            y2: {
               type: 'category',
                labels: activity.services.sort((a, b) => a.RTO - b.RTO).map(service => service.name),
                offset: true,
                position: 'left',
                stack: 't',
                border: {
